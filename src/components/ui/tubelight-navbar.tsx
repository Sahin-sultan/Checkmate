...
export function NavBar({ items, className, activeTab, onTabChange }: NavBarProps) {
    ...
    return (
        <motion.div
            ...
            transition={{
                duration: 0.8,
                bounce: 0.2, // Small bounce
                type: "spring"
            }}
            className={cn(
                "fixed bottom-0 left-1/2 z-50 mb-6 max-w-[95%]",
                className,
            )}
        >
                ...
                {items.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                "relative cursor-pointer text-base font-semibold px-2 sm:px-6 py-2 rounded-full transition-colors duration-500",
