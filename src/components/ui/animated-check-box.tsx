...
const NeonCheckbox: React.FC<NeonCheckboxProps> = ({
    ...
    return (
        <label
            className={cn(`relative inline-block w-[var(--size)] h-[var(--size)] cursor-pointer`, className)}
        ...
        >
            ...
            <div className="relative w-full h-full neon-checkbox__frame">
                <div className={`absolute inset-0 bg-transparent rounded border-2 transition-all duration-400 neon-checkbox__box ${isChecked
                    ...
                    <div className="absolute inset-[2px] flex items-center justify-center neon-checkbox__check-container">
                        <svg
                            viewBox="0 0 24 24"
                            className={`w-4/5 h-4/5 fill-none stroke-[var(--primary)] stroke-[3] stroke-linecap-round stroke-linejoin-round [stroke-dasharray:40] origin-center transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] neon-checkbox__check ${isChecked
                    ...
                    <div className={`absolute -inset-0.5 rounded-md bg-[var(--primary)] blur-md transition-opacity duration-400 neon-checkbox__glow ${isChecked ? 'opacity-20' : 'opacity-0'
                        }`} />
                    ...
                    <div className="absolute inset-0 rounded overflow-hidden neon-checkbox__borders">
                        {[...Array(4)].map((_, i) => (
                            <span
                                key={i}
                                className={`absolute w-10 h-px bg-[var(--primary)] transition-opacity duration-400 ${isChecked ? 'opacity-100' : 'opacity-0'
                ...
                <div className="neon-checkbox__effects">
                    <div className="absolute inset-0 neon-checkbox__particles">
                        {[...Array(12)].map((_, i) => (
                            <span
                                key={i}
                                className={`absolute w-1 h-1 bg-[var(--primary)] rounded-full pointer-events-none top-1/2 left-1/2 shadow-[0_0_6px_var(--primary)] ${isChecked ? 'animate-[particleExplosion_0.6s_ease-out_forwards]' : 'opacity-0'
                                    }`}
                                style={{
                                    '--x': [
                    ...
                    <div className="absolute -inset-5 pointer-events-none neon-checkbox__rings">
                        {[...Array(3)].map((_, i) => (
                            <div
                                key={i}
                                className={`absolute inset-0 rounded-full border border-[var(--primary)] scale-0 ring ${isChecked ? 'animate-[ringPulse_0.6s_ease-out_forwards]' : 'opacity-0'
                                    }`}
                    ...
                    <div className="absolute inset-0 neon-checkbox__sparks">
                        {[...Array(4)].map((_, i) => (
                            <span
                                key={i}
                                className={`absolute w-5 h-px bg-gradient-to-r from-[var(--primary)] to-transparent top-1/2 left-1/2 ${isChecked ? 'animate-[sparkFlash_0.6s_ease-out_forwards]' : 'opacity-0'
                                    }`}
